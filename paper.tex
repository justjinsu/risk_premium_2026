...
\begin{enumerate}
    \item \textbf{Korea Power Supply Plan Integration}: We incorporate official government coal dispatch trajectories from the 10th Power Supply Plan (전력수급기본계획), translating national policy into plant-level capacity factor reductions and revenue impacts.
...
\subsubsection{Korea National Power Supply Plan (전력수급기본계획)}
...
\begin{tabular}{lrrcr}
\toprule
Scenario & NPV (\$M) & $\Delta$ NPV & Rating & CRP (bps) \\
\midrule
Baseline & 8,964 & - & A & 0 \\
\midrule
\textit{Transition Risk Only} & & & & \\
\quad Moderate Transition & -520 & -106\% & B & 4,960 \\
\quad Aggressive Transition & -5,025 & -156\% & B & 7,167 \\
\midrule
\textit{Physical Risk Only} & & & & \\
\quad Moderate Physical (RCP 4.5) & 8,320 & -7\% & A & 315 \\
\quad High Physical (RCP 8.5) & 7,347 & -18\% & A & 792 \\
\midrule
\textit{Combined Risks} & & & & \\
\quad Combined Moderate & -836 & -109\% & B & 5,115 \\
\quad Combined Aggressive & -5,214 & -158\% & B & 7,259 \\
\midrule
\textit{Other Scenarios} & & & & \\
\quad Low Demand & 5,687 & -37\% & BBB & 1,675 \\
\quad Severe Drought & 3,808 & -58\% & BBB & 2,595 \\
\bottomrule
\end{tabular}
\end{table}
...
The most striking finding is that \textit{transition risk dominates physical risk by a factor of 5-6×}. Moderate transition scenarios (based on Korea's official 10th Power Supply Plan) reduce NPV by 106\%, while high physical climate risks (RCP 8.5) reduce NPV by only 18\%. This disparity demonstrates that \textbf{government policy—not climate change itself—is the primary driver of coal asset stranding in Korea}.
...
Combined, CLIMADA hazards reduce effective capacity factor by 5.85\% (RCP 8.5, 2050), translating to \$1.1 billion NPV loss and 792 bps CRP increase. While less severe than policy impacts, these risks are \textit{unavoidable} (adaptation costly, relocation impossible) and \textit{permanent} (sea level rise irreversible on human timescales).

\subsection{The Credit Rating Death Spiral}

\subsubsection{Investment Grade Loss at Policy-Driven Thresholds}

A critical non-linearity emerges at the investment grade boundary. All transition scenarios (moderate, aggressive, combined) trigger rating downgrades from A (150 bps spread) to B (600 bps spread), while physical-only scenarios maintain investment grade (A or BBB).

The threshold metric is EBITDA/Interest Expense coverage:
\begin{itemize}
    \item \textbf{Baseline}: 3.85× (A rating, safe)
    \item \textbf{Moderate Transition}: -1.60× (B rating, \textit{negative} EBITDA, cannot service debt)
    \item \textbf{High Physical}: 3.19× (A rating maintained)
\end{itemize}

Korea Power Plan dispatch reductions drive EBITDA negative (project becomes insolvent), while CLIMADA physical risks merely reduce positive EBITDA. This categorical difference explains the rating cliff.
...
Combined scenarios exhibit \textit{superadditive} risk effects. The worst-case scenario (aggressive transition + high physical + high carbon pricing) produces:

\begin{itemize}
    \item \textbf{NPV Loss}: -\$14.2 billion (158\% of baseline)
    \item \textbf{CRP}: 7,259 bps (72.6 percentage points)
    \item \textbf{WACC}: 6.75\% → 79.34\% (11.7× increase)
\end{itemize}

The combined impact (158\%) \textit{exceeds} the sum of individual risks:
\begin{itemize}
    \item Aggressive transition alone: -156\%
    \item High physical alone: -18\%
    \item Expected additive: -174\%
    \item Actual combined: -158\%
\end{itemize}

This apparent "subadditivity" is an artifact of NPV already being deeply negative under transition risk alone; physical risks cannot further reduce a negative NPV by much. However, the CRP shows true synergy:
\begin{itemize}
    \item Transition CRP: 7,167 bps
    \item Physical CRP: 792 bps
    \item Expected additive: 7,959 bps
    \item Actual combined: 7,259 bps (interaction term: -700 bps)
\end{itemize}

The negative interaction reflects credit rating floor effects: once rating reaches B (600 bps), further deterioration has bounded impact on spreads (cannot go below junk/default threshold without declaring bankruptcy).
...
The "Climate Risk Premium" embedded in Samcheok's 6-7\% bond yields suggests markets are \textit{beginning} to price policy risk, but our analysis indicates CRP should be 2,000-7,000 bps higher, implying either (a) market complacency or (b) implicit government bailout expectations.
...
The Samcheok Blue Power plant, commissioned in 2024 as South Korea's last coal project, exemplifies the financial risks of new fossil infrastructure in a climate-constrained world. Our integrated framework—combining Korea's National Power Supply Plan (전력수급기본계획), CLIMADA physical hazard data, and KIS credit rating methodology—quantifies three key findings:

\begin{enumerate}
    \item \textbf{Policy risk dominates physical risk}: Official government dispatch reductions reduce NPV by 106\%, while CLIMADA climate hazards reduce NPV by 18\%. Government energy policy—not climate change itself—is the primary driver of coal asset stranding in Korea.

    \item \textbf{Investment grade loss is inevitable}: All transition scenarios (moderate, aggressive, combined) trigger credit rating downgrades from A to B (investment grade loss), increasing debt spreads from 150 bps to 600 bps. Once EBITDA/Interest falls below 2.0×, recovery is impossible without extraordinary intervention.

    \item \textbf{Climate Risk Premium is material and quantifiable}: The CRP ranges from 315 bps (moderate physical risk) to 7,259 bps (combined aggressive scenario), making projects economically unviable. Samcheok's current 6-7\% bond yields suggest markets are beginning to price policy risk but remain complacent about downside scenarios.
\end{enumerate}
